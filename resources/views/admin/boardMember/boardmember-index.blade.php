    ...
    <div class="px-2 px-md-4">
        <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item" role="presentation">
                <a href="{{ route('boardmember-create') }}" class="text-decoration-none"><button class="nav-link">Add
                    Board Member</button></a>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="paid-tab" data-bs-toggle="tab" data-bs-target="#paid" type="button"
                    role="tab" aria-controls="paid" aria-selected="false">View Board Member</button>
            </li>
        </ul>
                    ...
                    <table class="table" style="width: 100%">
                        <thead>
                            <tr>
                                ...
                                <th scope="col" class="text-light" style="background-color: #515151">Content
                                </th>
                                <th scope="col" class="text-light" style="background-color: #515151">Is Board Member?
                                </th>
                        ...
                        <tbody>
                            ...
                            @endforeach --}}
                            @foreach ($boardmembers as $key => $boardmember)
                                <tr class=" text-center">
                                    <td scope="row">{{ $key + 1 }}</td>
                                    <td scope="row">{{ $boardmember->name }}</td>
                                    <td scope="row">{{ $boardmember->designation }}</td>
                                    <td scope="row">{{ $boardmember->content }}</td>
                                    <td scope="row">@if($boardmember->board_member == '1' ) Yes
                                        @else
                                        No 
                                        @endif
                                    </td>
                                    <td><img src="{{ asset('images/photos/' . $boardmember->image) }}"
                                            alt="{{ $boardmember->alt_text }}" height="100" width="100"></td>

                                    <td>
                                        <div class="container-fluid d-flex">


                                            <a class="mx-2" href="{{ route('boardmember-edit', $boardmember) }}">
                                                <div class="btn blue_button sized"><i
                                                        class='bx bx-message-square-edit'></i>edit
                                                </div>
                                            </a>
                                            <a class="mx-2" href="{{ route('boardmember-delete', $boardmember) }}">
                    ...
                    <div class="col-7 d-flex justify-content-end">
                        <div class="d-flex p-0 m-0">
                            <a class="text-decoration-none"
                                href="{{ $boardmembers->withQueryString()->previousPageUrl() }}">
                            ...
                            <div
                                ...
                                {{ $boardmembers->currentPage() }}

                            </div>


                            <a class="text-decoration-none" href="{{ $boardmembers->withQueryString()->nextPageUrl() }}">
...
@section('footer')
    <script>
        ...
        function go() {
            ...
            const url = new URL("{{ url('/') }}/boardmember-index");
